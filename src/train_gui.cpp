 ...
 */
void CcBuildWagon(const CommandCost &result, TileIndex tile, uint64 p1, uint64 p2)
{
...
static void TrainDetailsInfoTab(const Train *v, int left, int right, int y, byte line_number)
{
	...
	if (rvi->railveh_type == RAILVEH_WAGON) {
		...
		if ( show_speed && (speed = GetVehicleProperty(v, PROP_TRAIN_SPEED, rvi->max_speed))) {
			...
			DrawString(left, right, y, STR_VEHICLE_DETAILS_TRAIN_WAGON_VALUE_AND_SPEED, TC_FROMSTRING, SA_LEFT);
		} else
		...
		DrawString(left, right, y, STR_VEHICLE_DETAILS_TRAIN_ENGINE_BUILT_AND_VALUE);
			...
			case 2:
				...
				DrawString( left, right, y, STR_CURRENT_STATUS, TC_FROMSTRING, SA_LEFT);
				break;
			default: NOT_REACHED();
		}
	}
