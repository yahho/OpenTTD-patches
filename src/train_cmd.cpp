...
static bool ChooseTrainTrack (Train *v, RailPathPos origin, const PathMPos<RailPathPos> &pos, bool force_res, Trackdir *best_trackdir = NULL);
	...
	int y, EngineID engine, PaletteID pal, EngineImageType image_type)
{
	if (RailVehInfo(engine)->railveh_type == RAILVEH_MULTIHEAD) {
		...
		seqf.Draw (dpi, preferred_x - ScaleGUITrad(14), yf, pal, pal == PALETTE_CRASH);
		seqr.Draw (dpi, preferred_x + ScaleGUITrad(15), yr, pal, pal == PALETTE_CRASH);
	} else {
...
static void CheckNextTrainTile(Train *v)
{
	...
	if (ft.m_new.is_single() && ft.m_new.has_signal_along() &&
			...
			!HasReservedPos(ft.m_new)) {
		/* If the next tile is a PBS signal, try to make a reservation. */
		ChooseTrainTrack (v, pos, ft.m_new, false);
	}
}
 ...
 */
static Trackdir DoTrainPathfind(const Train *v, const RailPathPos &origin, bool do_track_reservation, PFResult *dest)
{
 ...
 */
static ExtendReservationResult ExtendTrainReservation(const Train *v, RailPathPos *origin)
{
	CFollowTrackRail ft(v, !_settings_game.pf.forbid_90_deg);
	ft.SetPos(*origin);
 ...
 */
static bool TryReserveSafeTrack(const Train *v, const RailPathPos &pos, bool override_tailtype)
{
...
/* choose a track */
static bool ChooseTrainTrack (Train *v, RailPathPos origin,
	const PathMPos<RailPathPos> &pos, bool force_res,
	Trackdir *best_trackdir)
{
	...
	if (pos.is_single()) {
		if (best_trackdir != NULL) *best_trackdir = pos.td;
	...
	if (do_track_reservation) {
		switch (ExtendTrainReservation(v, &origin)) {
			...
			case EXTEND_RESERVATION_FAILED:
				if (best_trackdir != NULL) *best_trackdir = FindFirstTrackdir (pos.trackdirs);
				return false;
	...
	if (best_trackdir != NULL && !pos.is_single()) {
		...
		*best_trackdir = next_trackdir != INVALID_TRACKDIR ? next_trackdir : FindFirstTrackdir (pos.trackdirs);
	...
	if (!res_dest.pos.is_valid_tile()) {
		...
		if (TryReserveSafeTrack(v, origin, false)) {
			if (best_trackdir != NULL && !pos.is_single()) {
				TrackBits res = GetReservedTrackbits (pos.tile);
				*best_trackdir = FindFirstTrackdir (TrackBitsToTrackdirBits(res) & pos.trackdirs);
			}
			TryReserveRailTrack(v->GetPos());
			if (change_signal) MarkTileDirtyByTile (pos.tile);
			return true;
		} else {
			FreeTrainTrackReservation (v, &origin);
			return false;
...
bool TryPathReserve(Train *v, bool mark_as_stuck, bool first_tile_okay)
{
	 ...
	 * make matters worse. */
	if (!FollowTrainReservation (v, &origin, true)) {
		if (mark_as_stuck) MarkTrainAsStuck(v);
		return false;
	}
...
static void TrainEnterStation(Train *v, StationID station)
{
	...
	if (!(st->had_vehicle_of_type & HVOT_TRAIN)) {
		...
		AddNewsItem<ArrivalNewsItem> (STR_NEWS_FIRST_TRAIN_ARRIVAL, v, st);
...
static bool CheckTrainCollision(Train *v)
{
	...
	AddNewsItem<VehicleNewsItem> (STR_NEWS_TRAIN_CRASH, NT_ACCIDENT,
			v->index, tcc.num);
...
static Trackdir TrainControllerChooseTrackdir(Train *v, TileIndex tile, DiagDirection enterdir, DiagDirection tsdir, bool check_90deg, bool reverse)
{
	...
	if (res_trackdirs != TRACKDIR_BIT_NONE) {
		...
		if (!ChooseTrainTrack (v, v->GetPos(),
				PathMPos<RailPathPos> (tile, trackdirbits),
...
bool TrainController(Train *v, Vehicle *nomove, bool reverse)
{
	...
	for (prev = v->Previous(); v != nomove; prev = v, v = v->Next()) {
		...
		if (v->trackdir == TRACKDIR_WORMHOLE) {
			...
			enterdir = DiagdirBetweenTiles(v->tile, gp.tile);
		...
		if (enterdir == INVALID_DIAGDIR) {
			...
			if (new_in_wormhole == WORMHOLE_BRIDGE) {
				...
				if (prev->tile == gp.tile) {
					 ...
					 */
					DiagDirection exitdir = DiagdirBetweenTiles(gp.tile, prev->tile);
				...
				assert(CheckCompatibleRail(v, gp.tile, TrackdirToTrack(chosen_trackdir)));
			}
			...
			if (new_in_wormhole != WORMHOLE_NONE) {
				...
				bool track_changed = !old_in_wormhole &&
					(GetRailType (v->tile, TrackdirToTrack(v->trackdir)) !=
						GetRailType (gp.tile, TrackdirToTrack(chosen_trackdir)));
		...
		if (old_in_wormhole && old_tile != gp.tile && v->Next() == NULL && maptile_is_rail_tunnel(v->tile) && maptile_has_tunnel_signal(v->tile, false)
		...
		if (!TryPathReserve(v)) {
			...
			if (HasBit(v->flags, VRF_TRAIN_STUCK) && v->wait_counter > 2 * _settings_game.pf.wait_for_pbs_path * DAY_TICKS) {
				...
				if (_settings_client.gui.lost_vehicle_warn && v->owner == _local_company) {
					AddNewsItem<VehicleAdviceNewsItem> (STR_NEWS_TRAIN_IS_STUCK, v->index);
				}
				v->wait_counter = 0;
			}
...
RailPathPos Train::GetPos() const
{
	if (this->vehstatus & VS_CRASHED) return RailPathPos();
	...
	if (this->trackdir == TRACKDIR_WORMHOLE) {
		...
		return RailPathPos(TILE_ADD(this->tile, TileOffsByDiagDir(rev)), DiagDirToDiagTrackdir(ReverseDiagDir(rev)), this->tile);
	}

	return RailPathPos(this->tile, this->GetTrackdir());
}

RailPathPos Train::GetReversePos() const
{
	if (this->vehstatus & VS_CRASHED) return RailPathPos();
	...
	if (this->trackdir == TRACKDIR_WORMHOLE) {
		...
		return RailPathPos(TILE_ADD(other_end, TileOffsByDiagDir(dir)), DiagDirToDiagTrackdir(ReverseDiagDir(dir)), other_end);
	}

	return RailPathPos(this->tile, ReverseTrackdir(this->GetTrackdir()));
}
