...
public:
	...
	SaveLoadWindow (const WindowDesc *desc, AbstractFileType abstract_filetype, FileOperation fop)
		...
		switch (widget) {
			...
			case WID_SL_DRIVES_DIRECTORIES_LIST: {
				...
				for (uint pos = this->vscroll->GetPosition(); pos < this->fios_items.Length(); pos++) {
					...
					DrawString (dpi, r.left + WD_FRAMERECT_LEFT, r.right - WD_FRAMERECT_RIGHT, y, item->title, _fios_colours[GetDetailedFileType(item->type)]);
					...
					if (click_count == 1) {
						if (this->selected != file) {
							...
							if (GetDetailedFileType(file->type) == DFT_GAME_FILE) {
								...
								LoadGame(name, FOP_CHECK, DFT_GAME_FILE, NO_DIRECTORY);
							}
