...
static void UpdateSignalsAroundSegment(SigInfo info)
{
	...
	while (_tbuset.Get(&tile, &trackdir)) {
		...
		/*} else if (sig == SIGTYPE_NORMAL) {
			if(true) { // standart block signal
				if(HasSignalOnTrackdir(tile, ReverseTrackdir(trackdir))){ //bi-directional block signal
					//Trackdir rt = ReverseTrackdir(trackdir);
					CFollowTrackRail ft(INVALID_OWNER, RAILTYPES_RAIL | RAILTYPES_ELECTRIC | RAILTYPES_MONO | RAILTYPES_MAGLEV, NULL);
					bool follow, railtype_rail, has_signals;
					RailTileType rtt;
					TileIndex t = tile;

					do {
						follow = ft.Follow(t, trackdir);
						printf("follow = %s\n", follow?"true":"false");
						//railtype_rail = IsTileType(ft.m_new_tile, MP_RAILWAY);
						//printf("railtype == MP_RAILWAY = %s\n", railtype_rail?"true":"false");
						rtt = GetRailTileType(ft.m_new_tile);
						printf("Rail tile type = %d\n----------\n", rtt);
						if(rtt == 3) break;
						has_signals = HasSignalOnTrackdir(ft.m_new_tile, trackdir);
						printf("has_signals = %s\n", has_signals?"true":"false");
						t = ft.m_new_tile;
					} while(follow && !has_signals && !ft.m_err && rtt != 3);
					if(rtt != 3 && HasSignalOnTrackdir(ft.m_new_tile, trackdir)) { //here is signal
						if(GetSignalType(ft.m_new_tile, TrackdirToTrack(trackdir)) == SIGTYPE_NORMAL)// standart block signal
							if(HasSignalOnTrackdir(ft.m_new_tile, ReverseTrackdir(trackdir))) {//bi-directional block signal
								newstate = GetSignalStateByTrackdir(ft.m_new_tile, trackdir);
								printf("newstate = %s\n", newstate == SIGNAL_STATE_RED?"red":"green");
							}
					} else if(rtt == 3)
						newstate = SIGNAL_STATE_GREEN;
				}
			}*/
