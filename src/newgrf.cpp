...
/**
 * Disable the GRF being loaded
 * @param message Error message or STR_NULL.
 * @return Error message of the GRF for further customisation.
 */
static GRFError *DisableCur (StringID message = STR_NULL)
{
	GRFConfig *config = _cur.grfconfig;
 ...
 * @param config GRFConfig to disable.
 */
static void DisableGrf (StringID message, GRFConfig *config)
{
	assert (config != NULL);
	assert (config != _cur.grfconfig);

	config->status = GCS_DISABLED;

	GRFFile *file = GetFileByGRFID (config->ident.grfid);
	if (file != NULL) ClearTemporaryNewGRFData(file);

	delete config->error;
	config->error = new GRFError (STR_NEWGRF_ERROR_MSG_FATAL, message);
	config->error->data = xstrdup (_cur.grfconfig->GetName());
}
	...
	for (int i = 0; i < numinfo; i++) {
		...
		switch (prop) {
			...
			case 0x0A: { // Set airport layout
				...
				uint new_num_layouts = buf->ReadByte(); // Number of layouts
				buf->Skip (4); // Total size of the definition

				for (uint j = 0; j < new_num_layouts; j++) {
					/* First byte is rotation. */
					const byte *p = buf->GetData (3);
					layouts[j].p = p++;
				...
				for (uint j = 0; j < as->num_table; j++) {
					...
					for (size_t k = 0; k < size; k++) {
						assert (p[0] != 0 || p[1] != 0x80);
						att[k].ti.x = *p++; // Offsets from northermost tile
						att[k].ti.y = *p++;
						att[k].gfx  = *p++;
...
static void StationMapSpriteGroup(ByteReader *buf, uint8 idcount)
{
	...
	for (uint c = 0; c < cidcount; c++) {
		...
		for (uint i = 0; i < idcount; i++) {
			...
			statspec->spritegroup[ctype] = _cur.spritegroups[groupid];
		}
	}
	...
	for (uint i = 0; i < idcount; i++) {
		...
		statspec->spritegroup[CT_DEFAULT] = _cur.spritegroups[groupid];
...
static void ObjectMapSpriteGroup(ByteReader *buf, uint8 idcount)
{
	...
	for (uint c = 0; c < cidcount; c++) {
		...
		for (uint i = 0; i < idcount; i++) {
			...
			spec->spritegroup[ctype] = _cur.spritegroups[groupid];
		}
	}
	...
	for (uint i = 0; i < idcount; i++) {
		...
		spec->spritegroup[0] = _cur.spritegroups[groupid];
...
{
	DisableGrf (STR_NEWGRF_ERROR_STATIC_GRF_CAUSES_DESYNC, c);
}
...
{
	_cur_parameter->name.add (langid, _cur.grfconfig->ident.grfid, false, str);
	return true;
}
...
{
	_cur_parameter->desc.add (langid, _cur.grfconfig->ident.grfid, true, str);
	return true;
}
