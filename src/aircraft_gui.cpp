	...
	int y, VehicleID selection, EngineImageType image_type)
{
	...
	seq.Draw (dpi, x, y + y_offs, pal, (v->vehstatus & VS_CRASHED) != 0);
	if (helicopter) {
		...
		rotor_seq.Draw (dpi, x, y + y_offs - heli_offs, PAL_NONE, false);
	}
	if (v->index == selection) {
		x += x_offs;
		y += UnScaleGUI(rect.top) + y_offs - heli_offs;
		DrawFrameRect (dpi, x - 1, y - 1, x + width + 1, y + UnScaleGUI(rect.bottom - rect.top + 1) + heli_offs + 1, COLOUR_WHITE, FR_BORDERONLY);
	}
}
