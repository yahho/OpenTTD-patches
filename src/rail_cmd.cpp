	...
	if (flags & DC_EXEC) {
		...
		if (v != NULL) {
			...
			if (!(((v->vehstatus & VS_STOPPED) && v->cur_speed == 0) || v->current_order.IsType(OT_LOADING)) ||
					!IsSafeWaitingPosition(v, v->tile, v->GetVehicleTrackdir(), _settings_game.pf.forbid_90_deg)) {
...
static void TileLoop_Track(TileIndex tile)
{
	...
	if (IsPlainRail(tile) && old_ground != RAIL_GROUND_BARREN) { // wait until bottom is green
		...
		for (DiagDirection d = DIAGDIR_BEGIN; d < DIAGDIR_END; d++) {
			...
			if (!IsValidTile(tile2) || IsHouseTile(tile2) || IsTileType(tile2, MP_INDUSTRY) ||
					IsRoadOrDepotTile(tile2) || (IsTileType(tile2, MP_OBJECT) && !IsOwnedLand(tile2)) || IsTileType(tile2, MP_TUNNELBRIDGE) || !IsTileOwner(tile2, owner)) {
				fences |= 1 << d;
			}
		}
