...
{
	return (PathNode*)this->closedlist_hash.Get(node->pos);
}
...
void AyStar::ClosedListAdd(const PathNode *node)
{
	...
	*new_node = *node;
	this->closedlist_hash.Set(node->node.pos, new_node);
}
...
{
	return (OpenListNode*)this->openlist_hash.Get(node->pos);
}
...
OpenListNode *AyStar::OpenListPop()
{
	...
	if (res != NULL) {
		this->openlist_hash.DeleteValue(res->path.node.pos);
	}
...
void AyStar::OpenListAdd(PathNode *parent, const AyStarNode *node, int f, int g)
{
	...
	this->openlist_hash.Set(node->pos, new_node);
