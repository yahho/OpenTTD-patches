...
public:
	...
	uint64 p1 = 0;
	while(true) {
		printf("\tinstruction id=%d; Instruction code=%d, after=%d\n", si->Id(), si->Opcode(), next);
		switch(si->Opcode()) {
			case PSO_SET_SIGNAL: {
				SB(p1, 0, 3, this->track);
				SB(p1, 3, 16, next);
	...
	virtual void OnPlaceObject(Point pt, TileIndex tile1)
	{
		if (this->IsWidgetLowered(PROGRAM_WIDGET_COPY_PROGRAM)) {
...
//			uint64 p1 = 0;
			...
			//printf("\tinstruction id=%d; Instruction code=%d\n", si->Id(), si->Opcode());
