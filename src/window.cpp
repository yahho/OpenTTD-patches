...
static bool IsGoodAutoPlace1(int left, int top, int width, int height, int toolbar_y, Point &pos)
{
	...
	if (left < 0 || top < toolbar_y || right > _screen_width || bottom > _screen_height) return false;
...
static Point GetAutoPlacePosition(int width, int height)
{
	...
	if (IsGoodAutoPlace1(rtl ? _screen_width - width : 0, toolbar_y, width, height, toolbar_y, pt)) return pt;
	 ...
	 * of the closebox
	 */
	int left = rtl ? _screen_width - width : 0, top = toolbar_y;
...
static Point LocalGetWindowPlacement(const WindowDesc *desc, int16 sm_width, int16 sm_height, int window_number)
{
	...
	if (desc->parent_cls != 0 /* WC_MAIN_WINDOW */ && (w = FindWindowById(desc->parent_cls, window_number)) != NULL) {
		...
		if (desc->parent_cls == WC_BUILD_TOOLBAR || desc->parent_cls == WC_SCEN_LAND_GEN) {
			...
			if (w->top + 3 * indent_y < _screen_height) {
				pt.y = w->top + indent_y;
				...
				if (_current_text_dir == TD_RTL) {
					...
					if (pt.x + default_width >= indent_close && pt.x + indent_resize <= _screen_width) return pt;
				} else {
					pt.x = min(w->left + indent_close, _screen_width - default_width);
					if (pt.x + default_width >= indent_resize && pt.x + indent_close <= _screen_width) return pt;
				}
			}
		}
	}
