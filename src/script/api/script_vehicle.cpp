...
/* static */ bool ScriptVehicle::SetName(VehicleID vehicle_id, Text *name)
{
	...
	sstring <MAX_CHAR_LENGTH * MAX_LENGTH_VEHICLE_NAME_CHARS> text;
	EnforcePreconditionDecodedText(false, name, &text);
	EnforcePreconditionCustomError(false, text.utf8length() < MAX_LENGTH_VEHICLE_NAME_CHARS, ScriptError::ERR_PRECONDITION_STRING_TOO_LONG);
