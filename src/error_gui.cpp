	...
	*this = data;
	for (size_t i = 0; i < lengthof(this->strings); i++) {
		if (this->strings[i] != NULL) {
			this->strings[i] = xstrdup(this->strings[i]);
		...
		switch (widget) {
			...
			case WID_EM_MESSAGE:
				...
				if (this->detailed_msg == INVALID_STRING_ID) {
					...
					DrawStringMultiLine (_cur_dpi, r.left + WD_FRAMETEXT_LEFT, r.right - WD_FRAMETEXT_RIGHT, top, bottom, this->summary_msg, TC_WHITE, SA_CENTER);

					bottom = r.bottom - WD_FRAMERECT_BOTTOM;
					top = bottom - this->height_detailed - extra;
					DrawStringMultiLine (_cur_dpi, r.left + WD_FRAMETEXT_LEFT, r.right - WD_FRAMETEXT_RIGHT, top, bottom, this->detailed_msg, TC_WHITE, SA_CENTER);
				}
...
void ShowErrorMessage(StringID summary_msg, StringID detailed_msg, WarningLevel wl, int x, int y, const GRFFile *textref_stack_grffile, uint textref_stack_size, const uint32 *textref_stack)
{
	...
	if (wl != WL_INFO) {
		...
		GetString (&buf, summary_msg);
		if (detailed_msg != INVALID_STRING_ID) {
			buf.append (' ');
			AppendString (&buf, detailed_msg);
		}

		if (textref_stack_size > 0) StopTextRefStackUsage();

		switch (wl) {
			case WL_WARNING: IConsolePrint(CC_WARNING, buf.c_str()); break;
			default:         IConsoleError(buf.c_str()); break;
		}
	}
