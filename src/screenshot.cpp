...
bool MakeSmallMapScreenshot(unsigned int width, unsigned int height, SmallMapWindow *window)
{
	_screenshot_name[0] = '\0';
	const ScreenshotFormat *sf = _screenshot_formats + _cur_screenshot_format;
	bool ret = sf->proc(MakeScreenshotName(SCREENSHOT_NAME, sf->extension), SmallMapCallback, window, width, height, BlitterFactoryBase::GetCurrentBlitter()->GetScreenDepth(), _cur_palette.palette);
	ShowScreenshotResultMessage(ret);
	return ret;
}
