	...
	for (int pass = 0; pass < 2; pass++) {
		...
		for (int count = ts.tile_table_count; count != 0; count--, ti++) {
			...
			if (pass == 0) {
				...
				if (direction == 1 && HasBridgeAbove(tile) &&
...
CommandCost CmdLevelLand(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	TileIterator *iter = HasBit(p2, 0) ? (TileIterator *)new DiagonalTileIterator(tile, p1) : new OrthogonalTileIterator(TileArea (tile, p1));
