...
void DrawAircraftEngine (BlitArea *dpi, int left, int right,
	int preferred_x, int y, EngineID engine, PaletteID pal,
	EngineImageType image_type)
{
	...
	if (!(AircraftVehInfo(engine)->subtype & AIR_CTOL)) {
		...
		rotor_seq.Draw (dpi, preferred_x, y - ScaleGUITrad(5), PAL_NONE, false);
	}
}
...
static bool AircraftController(Aircraft *v)
{
	...
	if (dist == 0) {
		...
		DirDiff dirdiff = DirDifference (ChangeDir (amd->direction, (DirDiff) rotation), v->direction);
	...
	do {
		...
		if (amd->flag & AMED_LAND) {
			if (st->airport.tile == INVALID_TILE) {
				...
				AircraftNextAirportPos_and_Order(v);
				/* get aircraft back on running altitude */
				SetAircraftPosition(v, gp.xx, gp.yy, GetAircraftFlightLevel(v));
				continue;
			}
