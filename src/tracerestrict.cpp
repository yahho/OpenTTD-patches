 ...
 * @p order may be NULL
 */
static bool TestOrderCondition(const BaseOrder *order, TraceRestrictItem item)
{
	bool result = false;
				...
				switch(type) {
					...
					case TRIT_COND_ENTRY_DIRECTION: {
						...
						switch (GetTraceRestrictValue(item)) {
							...
							case TRDTSV_FRONT:
								direction_match = IsRailwayTile(input.tile) && HasSignalOnTrackdir(input.tile, input.trackdir);
								break;
		...
		TileIndex source_tile, Track source_track, StringID error_msg)
{
	...
	DoCommandP(tile, p1, source_tile, CMD_PROGRAM_TRACERESTRICT_SIGNAL);
}
