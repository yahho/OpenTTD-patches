 ...
 */
CommandCost CmdBuildBridge(TileIndex end_tile, DoCommandFlag flags, uint64 p1, uint64 p2, const char *text)
{
 ...
 * @return the cost of this operation or an error
 */
CommandCost CmdBuildTunnel(TileIndex start_tile, DoCommandFlag flags, uint64 p1, uint64 p2, const char *text)
{
	CompanyID company = _current_company;
...
static CommandCost DoClearBridge(TileIndex tile, DoCommandFlag flags)
{
	...
	if (flags & DC_EXEC) {
		...
		for (TileIndex c = tile + delta; c != endtile; c += delta) {
			...
			ClearBridgeMiddle(c);
			MarkTileDirtyByTile(c, ZOOM_LVL_END);
		}
...
/*	if(IsTunnelBridgeExit(ti->tile)) {
		...
		switch(dir) {
			case DIAGDIR_NE: position = 1; break;
			case DIAGDIR_SE: position = 3; break;
			case DIAGDIR_SW: position = 0; break;
			case DIAGDIR_NW: position = 2; break;
			default: NOT_REACHED();
		}
	}*/
...
static void DrawBrigeSignalOnMiddelPart(const TileInfo *ti, TileIndex bridge_start_tile, uint z)
{
	...
	while (bridge_signal_position <= bridge_section) {
		bridge_signal_position += _settings_client.gui.drag_signals_density;
