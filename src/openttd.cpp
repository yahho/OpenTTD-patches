	...
	virtual void OnNewGRFsScanned()
	{
		...
		*save_config_ptr = save_config;
		...
		if (dedicated_host != NULL) {
			...
			*_network_bind_list.Append() = xstrdup(dedicated_host);
		}
...
int openttd_main(int argc, char *argv[])
{
	...
	while ((i = mgo.GetOpt()) != -1) {
		switch (i) {
		case 'I': free(graphics_set); graphics_set = xstrdup(mgo.opt); break;
		case 'S': free(sounds_set); sounds_set = xstrdup(mgo.opt); break;
		case 'M': free(music_set); music_set = xstrdup(mgo.opt); break;
		case 'm': free(musicdriver); musicdriver = xstrdup(mgo.opt); break;
		case 's': free(sounddriver); sounddriver = xstrdup(mgo.opt); break;
		case 'v': free(videodriver); videodriver = xstrdup(mgo.opt); break;
		case 'b': free(blitter); blitter = xstrdup(mgo.opt); break;
#if defined(ENABLE_NETWORK)
		case 'D':
			free(musicdriver);
			free(sounddriver);
			free(videodriver);
			free(blitter);
			musicdriver = xstrdup("null");
			sounddriver = xstrdup("null");
			videodriver = xstrdup("dedicated");
			blitter = xstrdup("null");
			dedicated = true;
		...
		case 'q': {
			...
			if (!res || _load_check_data.HasErrors()) {
				...
				if (_load_check_data.HasErrors()) {
					...
					GetString (buf, _load_check_data.error.str);
					fprintf(stderr, "%s\n", buf);
				}
				goto exit_noshutdown;
			}
		...
		case 'c': free(_config_file); _config_file = xstrdup(mgo.opt); break;
...
#ifdef ENABLE_NETWORK
				if (_network_server) {
					bstrfmt (_network_game_info.map_name, "%s (Loaded game)", _file_to_saveload.title);
				}
#endif /* ENABLE_NETWORK */
			}
			break;
...
#ifdef ENABLE_NETWORK
			if (_network_server) {
				bstrfmt (_network_game_info.map_name, "%s (Heightmap)", _file_to_saveload.title);
			}
#endif /* ENABLE_NETWORK */
