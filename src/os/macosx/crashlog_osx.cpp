...
class CrashLogOSX : public CrashLog {
	...
	sstring<MAX_PATH> filename_log;        ///< Path of crash.log
...
public:
	...
	CrashLogOSX(int signum) : signum(signum),
		filename_log(), filename_save(), filename_screenshot()
	{
	}
	...
	bool MakeCrashLog()
	{
		...
		printf("Writing crash savegame...\n");
		if (!this->WriteScreenshot (&filename_screenshot)) {
			filename_screenshot.clear();
			ret = false;
		}
	...
	void DisplayCrashDialog() const
	{
		...
		bstrfmt (message,
				 ...
				 "Generated file(s):\n%s\n%s\n%s",
				 this->filename_log.c_str(), this->filename_save.c_str(), this->filename_screenshot.c_str());
