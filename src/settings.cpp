 ...
 * @param type the type of elements the array holds (eg INT8, UINT16, etc.)
 * @return the string representation in newly-allocated storage
 */
 ...
 * @return the string representation in newly-allocated storage
 */
static char *MakeOneOfMany (const char *many, int id)
{
	int orig_id = id;
...
{
	uint options_to_load = GAME_DIFFICULTY_NUM - ((stv != NULL && stv->is_ottd_before (4)) ? 1 : 0);
	...
	for (uint i = 0; i < options_to_load; i++) {
		...
		if (!sd->save.is_currently_valid()) continue;
		void *var = sd->save.get_variable_address (stv != NULL ? &_settings_game : &_settings_newgame);
...
static GRFConfig *GRFLoadConfig(IniFile *ini, const char *grpname, bool is_static)
{
	...
	for (IniItem::const_iterator item = group->cbegin(); item != group->cend(); item++) {
		...
		if (!FillGRFDetails(c, is_static) || HasBit(c->flags, GCF_INVALID)) {
			...
			SetDParamStr(0, StrEmpty(filename) ? item->get_name() : filename);
...
static void GRFSaveConfig(IniFile *ini, const char *grpname, const GRFConfig *list)
{
	...
	for (c = list; c != NULL; c = c->next) {
		...
		sstring<4 * 2 + 1 + 16 * 2 + 1 + MAX_PATH> key;
		key.fmt ("%08X|", BSWAP32(c->ident.grfid));
		key.append_md5sum (c->ident.md5sum);
		key.append_fmt ("|%s", c->filename);

		sstring<512> params;
		GRFBuildParamList (&params, c);
		group->get_item(key.c_str())->SetValue(params.c_str());
	}
}
...
CommandCost CmdChangeSetting(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	if (!sd->save.is_currently_valid()) return CMD_ERROR;
...
bool SetSettingValue(uint index, int32 value, bool force_newgame)
{
	...
	if (sd->save.flags & SLF_NO_NETWORK_SYNC) {
		...
		if (_game_mode != GM_MENU) {
			void *var2 = sd->save.get_variable_address (&_settings_newgame);
...
void SetCompanySetting(uint index, int32 value)
{
	...
	if (Company::IsValidID(_local_company) && _game_mode != GM_MENU) {
		...
		void *var = sd->save.get_variable_address (&_settings_client.company);
		Write_ValidateSetting(var, sd, value);
		if (sd->desc.proc != NULL) sd->desc.proc((int32)ReadValue(var, sd->save.conv));
	}
}
...
void SetDefaultCompanySettings(CompanyID cid)
{
	...
	for (sd = _company_settings; sd->save.type != SL_END; sd++) {
		void *var = sd->save.get_variable_address (&c->settings);
...
void SyncCompanySettings()
{
	...
	for (sd = _company_settings; sd->save.type != SL_END; sd++, i++) {
		const void *old_var = sd->save.get_variable_address (&Company::Get(_current_company)->settings);
...
bool SetSettingValue(uint index, const char *value, bool force_newgame)
{
	...
	if (sd->save.length == 0) {
		...
		*var = strcmp(value, "(null)") == 0 ? NULL : xstrdup(value);
		...
		char *var = (char*) sd->save.get_variable_address();
...
void IConsoleListSettings(const char *prefilter)
{
	...
	for (const SettingDesc *sd = _settings; sd->save.type != SL_END; sd++) {
		...
		char value[80];
		const void *ptr = sd->save.get_variable_address (&GetGameSettings());
