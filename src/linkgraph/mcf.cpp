...
public:
	...
	private:
		/** Lookup table for getting NodeIDs from StationIDs. */
		std::vector<NodeID> *const station_to_node;
...
void MultiCommodityFlow::Dijkstra(NodeID source_node, PathVector &paths)
{
	...
	while (!annos.empty()) {
		...
		for (NodeID to = iter.Next(); to != INVALID_NODE; to = iter.Next()) {
			...
			LinkGraphJobEdge &edge = this->job[from][to];
			...
			uint distance = this->job.EdgeDistance (from, to) + 1;
 ...
 */
uint MultiCommodityFlow::PushFlow(LinkGraphJobEdge &edge, Path *path, uint accuracy,
		uint max_saturation)
{
...
void MCF1stPass::EliminateCycle(PathVector &path, Path *cycle_begin, uint flow)
{
	...
	do {
		...
		LinkGraphJobEdge &edge = this->job[prev][cycle_begin->GetNode()];
	...
	do {
		...
		for (NodeID source = 0; source < size; ++source) {
			...
			for (NodeID dest = 0; dest < size; ++dest) {
				LinkGraphJobEdge &edge = job[source][dest];
...
MCF2ndPass::MCF2ndPass(LinkGraphJob &job) : MultiCommodityFlow(job)
{
	...
	while (demand_left) {
		...
		for (NodeID source = 0; source < size; ++source) {
			...
			for (NodeID dest = 0; dest < size; ++dest) {
				LinkGraphJobEdge &edge = this->job[source][dest];
				Path *path = paths[dest];
