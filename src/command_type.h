...
enum DoCommandFlag {
	...
	DC_ALL_TILES             = 0x200, ///< allow this command also on void tiles
...
/**
 * Source of a command.
 * These are used to know what callback to run after a command finishes, if any.
 */
enum CommandSource {
	/* these are the possible original sources for a command */
	CMDSRC_SELF,          ///< the command was issued by the local player
	CMDSRC_AI,            ///< the command was issued by an AI script
	CMDSRC_GS,            ///< the command was generated by a game script
	CMDSRC_OTHER,         ///< skip running a callback for this command
	/* these mean that the command has already undergone network processing */
	CMDSRC_NETWORK_SELF,  ///< the command comes from the network, and is ours
	CMDSRC_NETWORK_AI,    ///< the command comes from the network, and was issued by an AI script
	CMDSRC_NETWORK_GS,    ///< the command comes from the network, and was generated by a game script
	CMDSRC_NETWORK_OTHER, ///< the command comes from the network, and is not ours, so skip any callback
	/* network flag, to transform an original source into a network-processed one */
	CMDSRC_NETWORK = CMDSRC_NETWORK_SELF,
};

/** Check if a command is local (does not come from the network). */
static inline bool cmdsrc_is_local (CommandSource cmdsrc)
{
	return cmdsrc < CMDSRC_NETWORK;
}

/** Get the basic command source for a (possibly network-processed) source. */
static inline CommandSource cmdsrc_get_type (CommandSource cmdsrc)
{
	/* we need CMDSRC_NETWORK to be a power of 2 for this to work as is */
	assert_compile (CMDSRC_NETWORK == 4);

	return (CommandSource) (cmdsrc & (CMDSRC_NETWORK - 1));
}

/** Get the network-processed equivalent of a command source. */
static inline CommandSource cmdsrc_make_network (CommandSource cmdsrc)
{
	assert (cmdsrc_is_local (cmdsrc));

	return (CommandSource) (cmdsrc + CMDSRC_NETWORK);
}
