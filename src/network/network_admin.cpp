...
{
	Packet p (ADMIN_PACKET_SERVER_ERROR);
	p.Send_uint8 (error);
	this->SendPacket (&p);
...
{
	Packet p (ADMIN_PACKET_SERVER_PROTOCOL);
...
{
	Packet p (ADMIN_PACKET_SERVER_WELCOME);
...
{
	Packet p (ADMIN_PACKET_SERVER_DATE);
	p.Send_uint32 (_date);
	this->SendPacket (&p);
...
{
	Packet p (ADMIN_PACKET_SERVER_CLIENT_JOIN);
...
NetworkRecvStatus ServerNetworkAdminSocketHandler::SendClientInfo(const NetworkClientSocket *cs, const NetworkClientInfo *ci)
{
	...
	Packet p (ADMIN_PACKET_SERVER_CLIENT_INFO);
...
{
	Packet p (ADMIN_PACKET_SERVER_CLIENT_UPDATE);
...
{
	Packet p (ADMIN_PACKET_SERVER_CLIENT_QUIT);
	p.Send_uint32 (client_id);
	this->SendPacket (&p);
...
{
	Packet p (ADMIN_PACKET_SERVER_CLIENT_ERROR);
	p.Send_uint32 (client_id);
	p.Send_uint8  (error);
	this->SendPacket (&p);
...
{
	Packet p (ADMIN_PACKET_SERVER_COMPANY_NEW);
...
{
	Packet p (ADMIN_PACKET_SERVER_COMPANY_REMOVE);
...
NetworkRecvStatus ServerNetworkAdminSocketHandler::SendCompanyEconomy()
{
	...
	FOR_ALL_COMPANIES(company) {
		...
		Packet p (ADMIN_PACKET_SERVER_COMPANY_ECONOMY);
...
NetworkRecvStatus ServerNetworkAdminSocketHandler::SendCompanyStats()
{
	...
	FOR_ALL_COMPANIES(company) {
		Packet p (ADMIN_PACKET_SERVER_COMPANY_STATS);
...
{
	Packet p (ADMIN_PACKET_SERVER_CHAT);
...
{
	Packet p (ADMIN_PACKET_SERVER_RCON_END);
...
{
	Packet p (ADMIN_PACKET_SERVER_RCON);
...
NetworkRecvStatus ServerNetworkAdminSocketHandler::Receive_ADMIN_RCON (RecvPacket *p)
{
...
NetworkRecvStatus ServerNetworkAdminSocketHandler::Receive_ADMIN_GAMESCRIPT (RecvPacket *p)
{
	if (this->status == ADMIN_STATUS_INACTIVE) return this->SendError(NETWORK_ERROR_NOT_EXPECTED);
...
NetworkRecvStatus ServerNetworkAdminSocketHandler::Receive_ADMIN_PING (RecvPacket *p)
{
	if (this->status == ADMIN_STATUS_INACTIVE) return this->SendError(NETWORK_ERROR_NOT_EXPECTED);
...
NetworkRecvStatus ServerNetworkAdminSocketHandler::SendConsole(const char *origin, const char *string)
{
	...
	Packet p (ADMIN_PACKET_SERVER_CONSOLE);
...
NetworkRecvStatus ServerNetworkAdminSocketHandler::SendGameScript(const char *json)
{
	...
	Packet p (ADMIN_PACKET_SERVER_GAMESCRIPT);
	p.Send_string (json);
	this->SendPacket (&p);
...
{
	Packet p (ADMIN_PACKET_SERVER_PONG);
	p.Send_uint32 (d1);
	this->SendPacket (&p);
...
{
	Packet p (ADMIN_PACKET_SERVER_CMD_NAMES);
...
{
	Packet p (ADMIN_PACKET_SERVER_CMD_LOGGING);
...
NetworkRecvStatus ServerNetworkAdminSocketHandler::Receive_ADMIN_JOIN (RecvPacket *p)
{
	if (this->status != ADMIN_STATUS_INACTIVE) return this->SendError(NETWORK_ERROR_NOT_EXPECTED);
...
NetworkRecvStatus ServerNetworkAdminSocketHandler::Receive_ADMIN_QUIT (RecvPacket *p)
{
	/* The admin is leaving nothing else to do */
	return this->CloseConnection();
}

NetworkRecvStatus ServerNetworkAdminSocketHandler::Receive_ADMIN_UPDATE_FREQUENCY (RecvPacket *p)
{
	if (this->status == ADMIN_STATUS_INACTIVE) return this->SendError(NETWORK_ERROR_NOT_EXPECTED);
...
NetworkRecvStatus ServerNetworkAdminSocketHandler::Receive_ADMIN_POLL (RecvPacket *p)
{
	if (this->status == ADMIN_STATUS_INACTIVE) return this->SendError(NETWORK_ERROR_NOT_EXPECTED);
...
NetworkRecvStatus ServerNetworkAdminSocketHandler::Receive_ADMIN_CHAT (RecvPacket *p)
{
	if (this->status == ADMIN_STATUS_INACTIVE) return this->SendError(NETWORK_ERROR_NOT_EXPECTED);
