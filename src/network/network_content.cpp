...
static char *GetFullFilename(const ContentInfo *ci, bool compressed)
{
	...
	uint n = FioGetFullPath (buf, lengthof(buf), SP_AUTODOWNLOAD_DIR, dir, ci->filename);
	if (n < lengthof(buf)) {
		snprintf (buf + n, lengthof(buf) - n, compressed ? ".tar.gz" : ".tar");
	}
...
void ClientNetworkContentSocketHandler::AfterDownload()
{
	...
	if (GunzipFile(this->curInfo)) {
		...
		if (this->curInfo->type == CONTENT_TYPE_BASE_MUSIC) {
			...
			TarCache::cache[BASESET_DIR].extract (GetFullFilename(this->curInfo, false));
