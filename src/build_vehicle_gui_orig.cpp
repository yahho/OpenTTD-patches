 ...
 */
uint GetEngineListHeightOrig(VehicleType type)
{
 ...
 */
int DrawVehiclePurchaseInfoOrig(int left, int right, int y, EngineID engine_number)
{
	const Engine *e = Engine::Get(engine_number);
	YearMonthDay ymd;
 ...
 */
void DrawEngineListOrig(VehicleType type, int l, int r, int y, const GUIEngineList *eng_list, uint16 min, uint16 max, EngineID selected_id, bool show_count, GroupID selected_group)
{
	...
	int step_size = GetEngineListHeightOrig(type);
...
struct BuildVehicleWindowOrig : Window {
	...
	BuildVehicleWindowOrig(WindowDesc *desc, TileIndex tile, VehicleType type) : Window(desc)
	{
	...
	virtual void UpdateWidgetSize(int widget, Dimension *size, const Dimension &padding, Dimension *fill, Dimension *resize)
	{
		switch (widget) {
			case WID_BV_LIST:
				resize->height = GetEngineListHeightOrig(this->vehicle_type);
	...
	virtual void DrawWidget(const Rect &r, int widget) const
	{
		switch (widget) {
			case WID_BV_LIST:
				DrawEngineListOrig(this->vehicle_type, r.left + WD_FRAMERECT_LEFT, r.right - WD_FRAMERECT_RIGHT, r.top + WD_FRAMERECT_TOP, &this->eng_list, this->vscroll->GetPosition(), min(this->vscroll->GetPosition() + this->vscroll->GetCapacity(), this->eng_list.Length()), this->sel_engine, false, DEFAULT_GROUP);
				break;
	...
	virtual void OnPaint()
	{
		...
		if (!this->IsShaded()) {
			...
			if (this->sel_engine != INVALID_ENGINE) {
				...
				int text_end = DrawVehiclePurchaseInfoOrig(nwi->pos_x + WD_FRAMETEXT_LEFT, nwi->pos_x + nwi->current_x - WD_FRAMETEXT_RIGHT,
...
void ShowBuildVehicleWindowOrig(TileIndex tile, VehicleType type)
{
	...
	new BuildVehicleWindowOrig(&_build_vehicle_desc, tile, type);
}
