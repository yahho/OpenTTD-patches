...
void Vehicle::PreDestructor()
{
	...
	if (this->type == VEH_ROAD && this->IsPrimaryVehicle()) {
		...
		if (!(v->vehstatus & VS_CRASHED) && IsInsideMM(v->state, RVSB_IN_DT_ROAD_STOP, RVSB_IN_DT_ROAD_STOP_END)) {
			...
			RoadStop::GetByTile(v->tile, GetRoadStopType(v->tile))->LeaveDriveThrough (TrackdirToExitdir((Trackdir)(v->state & RVSB_ROAD_STOP_TRACKDIR_MASK)), v->gcache.cached_total_length);
		}
	}
...
void VehicleEnterDepot(Vehicle *v)
{
	...
	if (v->current_order.IsType(OT_GOTO_DEPOT)) {
		...
		if (v->current_order.IsRefit()) {
			assert (!v->current_order.IsAutoRefit());
...
void Vehicle::ShowVisualEffect() const
{
	...
	do {
		...
		if (effect_model == VESM_NONE ||
				v->vehstatus & VS_HIDDEN ||
				HasBridgeAbove(v->tile) ||
				IsDepotTile(v->tile) ||
				IsTunnelTile(v->tile) ||
				(v->type == VEH_TRAIN &&
				!HasPowerOnRail(Train::From(v)->railtype, Train::From(v)->GetTrackRailType()))) {
			continue;
		}
