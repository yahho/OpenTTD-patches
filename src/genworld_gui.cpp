...
static const NWidgetPart _nested_generate_landscape_widgets[] = {
	...
	NWidget(WWT_PANEL, COLOUR_BROWN),
		...
		NWidget(NWID_HORIZONTAL), SetPIP(10, 0, 10),
			...
			NWidget(WWT_IMGBTN_2, COLOUR_ORANGE, WID_GL_TEMPERATE), SetMinimalSize(76, 54), SetDataTip(SPR_SELECT_TEMPERATE, STR_INTRO_TOOLTIP_TEMPERATE),
			NWidget(NWID_SPACER), SetFill(1, 0),
			NWidget(WWT_IMGBTN_2, COLOUR_ORANGE, WID_GL_ARCTIC), SetMinimalSize(76, 54), SetDataTip(SPR_SELECT_SUB_ARCTIC, STR_INTRO_TOOLTIP_SUB_ARCTIC_LANDSCAPE),
			NWidget(NWID_SPACER), SetFill(1, 0),
			NWidget(WWT_IMGBTN_2, COLOUR_ORANGE, WID_GL_TROPICAL), SetMinimalSize(76, 54), SetDataTip(SPR_SELECT_SUB_TROPICAL, STR_INTRO_TOOLTIP_SUB_TROPICAL_LANDSCAPE),
			NWidget(NWID_SPACER), SetFill(1, 0),
			NWidget(WWT_IMGBTN_2, COLOUR_ORANGE, WID_GL_TOYLAND), SetMinimalSize(76, 54), SetDataTip(SPR_SELECT_TOYLAND, STR_INTRO_TOOLTIP_TOYLAND_LANDSCAPE),
			NWidget(NWID_SPACER), SetFill(1, 0),
		EndContainer(),
...
struct GenerateLandscapeWindow : public Window {
		...
		: Window (desc), widget_id (0), x (0), y (0), seed_editbox ()
	{
		...
		this->seed_editbox.Print("%u", _settings_newgame.game_creation.generation_seed);
	...
	void DrawWidget (BlitArea *dpi, const Rect &r, int widget) const OVERRIDE
	{
		switch (widget) {
			case WID_GL_HEIGHTMAP_NAME_TEXT: {
				DrawString (dpi, r.left, r.right, r.top, this->name, TC_ORANGE);
				break;
			}
		}
	}
	...
	virtual void OnEditboxChanged(int wid)
	{
		if (wid == WID_GL_RANDOM_EDITBOX) {
			...
			_settings_newgame.game_creation.generation_seed = minu(strtoul(this->seed_editbox.GetText(), NULL, 10), UINT32_MAX - 1);
		}
	}
...
static const NWidgetPart _nested_create_scenario_widgets[] = {
	...
	NWidget(WWT_PANEL, COLOUR_BROWN),
		...
		NWidget(NWID_HORIZONTAL), SetPIP(10, 3, 10),
			NWidget(WWT_IMGBTN_2, COLOUR_ORANGE, WID_CS_TEMPERATE), SetMinimalSize(76, 54), SetDataTip(SPR_SELECT_TEMPERATE, STR_INTRO_TOOLTIP_TEMPERATE),
			NWidget(WWT_IMGBTN_2, COLOUR_ORANGE, WID_CS_ARCTIC), SetMinimalSize(76, 54), SetDataTip(SPR_SELECT_SUB_ARCTIC, STR_INTRO_TOOLTIP_SUB_ARCTIC_LANDSCAPE),
			NWidget(WWT_IMGBTN_2, COLOUR_ORANGE, WID_CS_TROPICAL), SetMinimalSize(76, 54), SetDataTip(SPR_SELECT_SUB_TROPICAL, STR_INTRO_TOOLTIP_SUB_TROPICAL_LANDSCAPE),
			NWidget(WWT_IMGBTN_2, COLOUR_ORANGE, WID_CS_TOYLAND), SetMinimalSize(76, 54), SetDataTip(SPR_SELECT_TOYLAND, STR_INTRO_TOOLTIP_TOYLAND_LANDSCAPE),
		EndContainer(),
	...
	void DrawWidget (BlitArea *dpi, const Rect &r, int widget) const OVERRIDE
	{
		switch (widget) {
			case WID_GP_PROGRESS_BAR:
				...
				DrawFrameRect (dpi, r.left + 1, r.top + 1, (int)((r.right - r.left - 2) * _gws.percent / 100) + r.left + 1, r.bottom - 1, COLOUR_MAUVE, FR_NONE);
			...
			case WID_GP_PROGRESS_TEXT:
				...
				DrawString (dpi, r.left, r.right, r.top, _gws.cls, TC_FROMSTRING, SA_HOR_CENTER);

				/* And say where we are in that class */
				SetDParam(0, _gws.current);
				SetDParam(1, _gws.total);
				DrawString (dpi, r.left, r.right, r.top + FONT_HEIGHT_NORMAL + WD_PAR_VSEP_NORMAL, STR_GENERATION_PROGRESS_NUM, TC_FROMSTRING, SA_HOR_CENTER);
		}
	}
};
