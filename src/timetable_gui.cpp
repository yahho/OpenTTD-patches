...
{
	DoCommandP(0, w->window_number, date, CMD_SET_TIMETABLE_START);
}
...
struct TimetableWindow : Window {
		...
		switch (widget) {
			case WID_VT_TIMETABLE_PANEL: {
				...
				while (order != NULL) {
					...
					if (i % 2 == 0) {
						DrawOrderString (v, order, order_id, _cur_dpi, y, i == selected, true, r.left + WD_FRAMERECT_LEFT, middle, r.right - WD_FRAMERECT_RIGHT);
						...
						DrawString (_cur_dpi, rtl ? r.left + WD_FRAMERECT_LEFT : middle, rtl ? middle : r.right - WD_FRAMERECT_LEFT, y, string, colour);
				...
				for (int i = this->vscroll->GetPosition(); i / 2 < v->GetNumOrders(); ++i) { // note: i is also incremented in the loop
					...
					if (i % 2 == 0) {
						if (arr_dep[i / 2].arrival != INVALID_TICKS) {
							DrawString (_cur_dpi, abbr_left, abbr_right, y, STR_TIMETABLE_ARRIVAL_ABBREVIATION, i == selected ? TC_WHITE : TC_BLACK);
							if (this->show_expected && i / 2 == earlyID) {
								SetDParam(0, _date + arr_dep[i / 2].arrival / DAY_TICKS);
								DrawString (_cur_dpi, time_left, time_right, y, STR_JUST_DATE_TINY, TC_GREEN);
							} else {
								SetDParam(0, _date + (arr_dep[i / 2].arrival + offset) / DAY_TICKS);
								DrawString (_cur_dpi, time_left, time_right, y, STR_JUST_DATE_TINY,
										show_late ? TC_RED : i == selected ? TC_WHITE : TC_BLACK);
							}
						}
					} else {
						if (arr_dep[i / 2].departure != INVALID_TICKS) {
							DrawString (_cur_dpi, abbr_left, abbr_right, y, STR_TIMETABLE_DEPARTURE_ABBREVIATION, i == selected ? TC_WHITE : TC_BLACK);
			...
			case WID_VT_SUMMARY_PANEL: {
				...
				if (total_time != 0) {
					...
					DrawString (_cur_dpi, r.left + WD_FRAMERECT_LEFT, r.right - WD_FRAMERECT_RIGHT, y, v->orders.list->IsCompleteTimetable() ? STR_TIMETABLE_TOTAL_TIME : STR_TIMETABLE_TOTAL_TIME_INCOMPLETE);
				}
				y += FONT_HEIGHT_NORMAL;
				...
				if (v->timetable_start != 0) {
					...
					DrawString (_cur_dpi, r.left + WD_FRAMERECT_LEFT, r.right - WD_FRAMERECT_RIGHT, y, STR_TIMETABLE_STATUS_START_AT);
				} else if (!HasBit(v->vehicle_flags, VF_TIMETABLE_STARTED)) {
					/* We aren't running on a timetable yet, so how can we be "on time"
					 * when we aren't even "on service"/"on duty"? */
					DrawString (_cur_dpi, r.left + WD_FRAMERECT_LEFT, r.right - WD_FRAMERECT_RIGHT, y, STR_TIMETABLE_STATUS_NOT_STARTED);
				} else if (v->lateness_counter == 0 || (!_settings_client.gui.timetable_in_ticks && v->lateness_counter / DAY_TICKS == 0)) {
					DrawString (_cur_dpi, r.left + WD_FRAMERECT_LEFT, r.right - WD_FRAMERECT_RIGHT, y, STR_TIMETABLE_STATUS_ON_TIME);
				} else {
					SetTimetableParams(0, 1, abs(v->lateness_counter));
					DrawString (_cur_dpi, r.left + WD_FRAMERECT_LEFT, r.right - WD_FRAMERECT_RIGHT, y, v->lateness_counter < 0 ? STR_TIMETABLE_STATUS_EARLY : STR_TIMETABLE_STATUS_LATE);
				}
				break;
			}
		}
	...
	virtual void OnClick(Point pt, int widget, int click_count)
	{
		...
		switch (widget) {
			...
			case WID_VT_CLEAR_TIME: { // Clear waiting time.
				...
				DoCommandP(0, p1, 0, CMD_CHANGE_TIMETABLE);
				break;
			}

			case WID_VT_CLEAR_SPEED: { // Clear max speed button.
				uint32 p1 = PackTimetableArgs(v, this->sel_index, true);
				DoCommandP(0, p1, UINT16_MAX, CMD_CHANGE_TIMETABLE);
				break;
			}

			case WID_VT_RESET_LATENESS: // Reset the vehicle's late counter.
				DoCommandP(0, v->index, 0, CMD_SET_VEHICLE_ON_TIME);
				break;
			...
			case WID_VT_AUTOFILL: { // Autofill the timetable.
				...
				DoCommandP(0, v->index, p2, CMD_AUTOFILL_TIMETABLE);
				break;
			}
	...
	virtual void OnQueryTextFinished(char *str)
	{
		...
		DoCommandP(0, p1, p2, CMD_CHANGE_TIMETABLE);
	}
