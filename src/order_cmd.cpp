...
void CheckOrders(const Vehicle *v)
{
	...
	if (v->owner == _local_company && v->day_counter % 20 == 0) {
			...
			if (order->IsType(OT_GOTO_STATION)) {
				...
				if (!CanVehicleUseStation(v, st)) {
					message = STR_NEWS_VEHICLE_HAS_INVALID_ENTRY;
							...
							(st->airport.GetFTA()->flags & AirportFTA::SHORT_STRIP) &&
		...
		AddNewsItem<VehicleAdviceNewsItem> (message, v->index);
	}
}
...
bool ProcessOrders(Vehicle *v)
{
	...
	if (order->Equals(v->current_order) && (v->type == VEH_AIRCRAFT || v->dest_tile != 0) &&
			(v->type != VEH_SHIP || !order->IsType(OT_GOTO_STATION) || Station::Get(order->GetDestination())->docks != NULL)) {
		return false;
	}
