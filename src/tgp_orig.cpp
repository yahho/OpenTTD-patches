...
static const int height_decimal_bits_orig = 4;
static const height_t _invalid_height_orig = -32768;

/** Fixed point array for amplitudes (and percent values) */
typedef int amplitude_t;
static const int amplitude_decimal_bits_orig = 10;
...
static HeightMap _height_map_orig = {NULL, 0, 0, 0, 0};

/** Conversion: int to height_t */
#define I2H(i) ((i) << height_decimal_bits_orig)
/** Conversion: height_t to int */
#define H2I(i) ((i) >> height_decimal_bits_orig)

/** Conversion: int to amplitude_t */
#define I2A(i) ((i) << amplitude_decimal_bits_orig)
/** Conversion: amplitude_t to int */
#define A2I(i) ((i) >> amplitude_decimal_bits_orig)

/** Conversion: amplitude_t to height_t */
#define A2H(a) ((a) >> (amplitude_decimal_bits_orig - height_decimal_bits_orig))


/** Walk through all items of _height_map.h */
#define FOR_ALL_TILES_IN_HEIGHT(h) for (h = _height_map_orig.h; h < &_height_map_orig.h[_height_map_orig.total_size]; h++)

/** Maximum index into array of noise amplitudes */
static const int TGP_FREQUENCY_MAX_ORIG = 6;

/**
 * Noise amplitudes (multiplied by 1024)
 * - indexed by "smoothness setting" and log2(frequency)
 */
static const amplitude_t _amplitudes_by_smoothness_and_frequency_orig[4][TGP_FREQUENCY_MAX_ORIG + 1] = {
...
static const amplitude_t _water_percent_orig[4] = {20, 80, 250, 400};

/** Desired maximum height - indexed by _settings_game.difficulty.terrain_type */
static const int8 _max_height_orig[4] = {
	6,       ///< Very flat
	9,       ///< Flat
	12,      ///< Hilly
	15,      ///< Mountainous
 ...
 */
static inline bool IsValidXYOrig(uint x, uint y)
{
	return ((int)x) >= 0 && x < _height_map_orig.size_x && ((int)y) >= 0 && y < _height_map_orig.size_y;
}
 ...
 */
static inline bool AllocHeightMapOrig()
{
	height_t *h;

	_height_map_orig.size_x = MapSizeX();
	_height_map_orig.size_y = MapSizeY();

	/* Allocate memory block for height map row pointers */
	_height_map_orig.total_size = (_height_map_orig.size_x + 1) * (_height_map_orig.size_y + 1);
	_height_map_orig.dim_x = _height_map_orig.size_x + 1;
	_height_map_orig.h = CallocT<height_t>(_height_map_orig.total_size);

	/* Iterate through height map initialize values */
	FOR_ALL_TILES_IN_HEIGHT(h) *h = _invalid_height_orig;

	return true;
}

/** Free height map */
static inline void FreeHeightMapOrig()
{
	if (_height_map_orig.h == NULL) return;
	free(_height_map_orig.h);
	_height_map_orig.h = NULL;
}
 ...
 */
static inline height_t RandomHeightOrig(amplitude_t rMax)
{
 ...
 */
static bool ApplyNoiseOrig(uint log_frequency, amplitude_t amplitude)
{
	uint size_min = min(_height_map_orig.size_x, _height_map_orig.size_y);
	uint step = size_min >> log_frequency;
	uint x, y;

	/* Trying to apply noise to uninitialized height map */
	assert(_height_map_orig.h != NULL);
	...
	if (log_frequency == 0) {
		...
		for (y = 0; y <= _height_map_orig.size_y; y += step) {
			for (x = 0; x <= _height_map_orig.size_x; x += step) {
				height_t height = (amplitude > 0) ? RandomHeightOrig(amplitude) : 0;
				_height_map_orig.height(x, y) = height;
			}
		}
		return true;
	}
	...
	for (y = 0; y <= _height_map_orig.size_y; y += 2 * step) {
		for (x = 0; x < _height_map_orig.size_x; x += 2 * step) {
			height_t h00 = _height_map_orig.height(x + 0 * step, y);
			height_t h02 = _height_map_orig.height(x + 2 * step, y);
			height_t h01 = (h00 + h02) / 2;
			_height_map_orig.height(x + 1 * step, y) = h01;
		}
	}
	...
	for (y = 0; y < _height_map_orig.size_y; y += 2 * step) {
		for (x = 0; x <= _height_map_orig.size_x; x += step) {
			height_t h00 = _height_map_orig.height(x, y + 0 * step);
			height_t h20 = _height_map_orig.height(x, y + 2 * step);
			height_t h10 = (h00 + h20) / 2;
			_height_map_orig.height(x, y + 1 * step) = h10;
		}
	}
	...
	for (y = 0; y <= _height_map_orig.size_y; y += step) {
		for (x = 0; x <= _height_map_orig.size_x; x += step) {
			_height_map_orig.height(x, y) += RandomHeightOrig(amplitude);
		}
	}
...
static void HeightMapGenerateOrig()
{
	uint size_min = min(_height_map_orig.size_x, _height_map_orig.size_y);
	...
	for (log_size_min = TGP_FREQUENCY_MAX_ORIG; (1U << log_size_min) < size_min; log_size_min++) { }
	log_frequency_min = log_size_min - TGP_FREQUENCY_MAX_ORIG;
	...
	do {
		...
		if (log_frequency >= 0) {
			...
			assert(log_frequency <= TGP_FREQUENCY_MAX_ORIG);
			amplitude = _amplitudes_by_smoothness_and_frequency_orig[_settings_game.game_creation.tgen_smoothness][log_frequency];
		} else {
			/* Amplitude for the low frequencies on big maps is 0, i.e. initialise with zero height */
			amplitude = 0;
		}
		continue_iteration = ApplyNoiseOrig(iteration_round, amplitude);
		iteration_round++;
...
static void HeightMapGetMinMaxAvgOrig(height_t *min_ptr, height_t *max_ptr, height_t *avg_ptr)
{
	height_t h_min, h_max, h_avg, *h;
	int64 h_accu = 0;
	h_min = h_max = _height_map_orig.height(0, 0);
	...
	/* Get average height */
	h_avg = (height_t)(h_accu / (_height_map_orig.size_x * _height_map_orig.size_y));
...
static int *HeightMapMakeHistogramOrig(height_t h_min, height_t h_max, int *hist_buf)
{
...
static void HeightMapSineTransformOrig(height_t h_min, height_t h_max)
{
	height_t *h;
...
static const control_point_t _curve_map_1_orig[] = {
	{ 0, 0 }, { 48, 24 }, { 192, 32 }, { 240, 96 }
};

static const control_point_t _curve_map_2_orig[] = {
	{ 0, 0 }, { 16, 24 }, { 128, 32 }, { 192, 64 }, { 240, 144 }
};

static const control_point_t _curve_map_3_orig[] = {
	{ 0, 0 }, { 16, 24 }, { 128, 64 }, { 192, 144 }, { 240, 192 }
};

static const control_point_t _curve_map_4_orig[] = {
	{ 0, 0 }, { 16, 24 }, { 96, 72 }, { 160, 192 }, { 220, 239 }, { 240, 239 }
};

static const control_point_list_t _curve_maps_orig[] = {
	{ lengthof(_curve_map_1_orig), _curve_map_1_orig },
	{ lengthof(_curve_map_2_orig), _curve_map_2_orig },
	{ lengthof(_curve_map_3_orig), _curve_map_3_orig },
	{ lengthof(_curve_map_4_orig), _curve_map_4_orig },
};

static void HeightMapCurvesOrig(uint level)
{
	height_t ht[lengthof(_curve_maps_orig)];
	...
	for (uint i = 0; i < sx * sy; i++) {
		c[i] = Random() % lengthof(_curve_maps_orig);
	}

	/* Apply curves */
	for (uint x = 0; x < _height_map_orig.size_x; x++) {

		/* Get our X grid positions and bi-linear ratio */
		float fx = (float)(sx * x) / _height_map_orig.size_x + 0.5f;
		uint x1 = (uint)fx;
		uint x2 = x1;
		...
		for (uint y = 0; y < _height_map_orig.size_y; y++) {

			/* Get our Y grid position and bi-linear ratio */
			float fy = (float)(sy * y) / _height_map_orig.size_y + 0.5f;
			uint y1 = (uint)fy;
			uint y2 = y1;
			...
			height_t *h = &_height_map_orig.height(x, y);

			/* Apply all curve maps that are used on this tile. */
			for (uint t = 0; t < lengthof(_curve_maps_orig); t++) {
				if (!HasBit(corner_bits, t)) continue;

				const control_point_t *cm = _curve_maps_orig[t].list;
				for (uint i = 0; i < _curve_maps_orig[t].length - 1; i++) {
...
static void HeightMapAdjustWaterLevelOrig(amplitude_t water_percent, height_t h_max_new)
{
	...
	HeightMapGetMinMaxAvgOrig(&h_min, &h_max, &h_avg);

	/* Allocate histogram buffer and clear its cells */
	int *hist_buf = CallocT<int>(h_max - h_min + 1);
	/* Fill histogram */
	hist = HeightMapMakeHistogramOrig(h_min, h_max, hist_buf);

	/* How many water tiles do we want? */
	desired_water_tiles = A2I(((int64)water_percent) * (int64)(_height_map_orig.size_x * _height_map_orig.size_y));
...
static double perlin_coast_noise_2D_orig(const double x, const double y, const double p, const int prime);
 ...
 */
static void HeightMapCoastLinesOrig(uint8 water_borders)
{
	...
	/* Lower to sea level */
	for (y = 0; y <= _height_map_orig.size_y; y++) {
		if (HasBit(water_borders, BORDER_NE)) {
			/* Top right */
			max_x = abs((perlin_coast_noise_2D_orig(_height_map_orig.size_y - y, y, 0.9, 53) + 0.25) * 5 + (perlin_coast_noise_2D_orig(y, y, 0.35, 179) + 1) * 12);
			max_x = max((smallest_size * smallest_size / 16) + max_x, (smallest_size * smallest_size / 16) + margin - max_x);
			if (smallest_size < 8 && max_x > 5) max_x /= 1.5;
			for (x = 0; x < max_x; x++) {
				_height_map_orig.height(x, y) = 0;
			}
		}

		if (HasBit(water_borders, BORDER_SW)) {
			/* Bottom left */
			max_x = abs((perlin_coast_noise_2D_orig(_height_map_orig.size_y - y, y, 0.85, 101) + 0.3) * 6 + (perlin_coast_noise_2D_orig(y, y, 0.45,  67) + 0.75) * 8);
			max_x = max((smallest_size * smallest_size / 16) + max_x, (smallest_size * smallest_size / 16) + margin - max_x);
			if (smallest_size < 8 && max_x > 5) max_x /= 1.5;
			for (x = _height_map_orig.size_x; x > (_height_map_orig.size_x - 1 - max_x); x--) {
				_height_map_orig.height(x, y) = 0;
			}
		}
	}
	...
	/* Lower to sea level */
	for (x = 0; x <= _height_map_orig.size_x; x++) {
		if (HasBit(water_borders, BORDER_NW)) {
			/* Top left */
			max_y = abs((perlin_coast_noise_2D_orig(x, _height_map_orig.size_y / 2, 0.9, 167) + 0.4) * 5 + (perlin_coast_noise_2D_orig(x, _height_map_orig.size_y / 3, 0.4, 211) + 0.7) * 9);
		...
		if (HasBit(water_borders, BORDER_SE)) {
			/* Bottom right */
			max_y = abs((perlin_coast_noise_2D_orig(x, _height_map_orig.size_y / 3, 0.85, 71) + 0.25) * 6 + (perlin_coast_noise_2D_orig(x, _height_map_orig.size_y / 3, 0.35, 193) + 0.75) * 12);
			max_y = max((smallest_size * smallest_size / 16) + max_y, (smallest_size * smallest_size / 16) + margin - max_y);
			if (smallest_size < 8 && max_y > 5) max_y /= 1.5;
			for (y = _height_map_orig.size_y; y > (_height_map_orig.size_y - 1 - max_y); y--) {
				_height_map_orig.height(x, y) = 0;
			}
		}
	}
...
static void HeightMapSmoothCoastInDirectionOrig(int org_x, int org_y, int dir_x, int dir_y)
{
	...
	assert(IsValidXYOrig(org_x, org_y));

	/* Search for the coast (first non-water tile) */
	for (x = org_x, y = org_y, ed = 0; IsValidXYOrig(x, y) && ed < max_coast_dist_from_edge; x += dir_x, y += dir_y, ed++) {
		/* Coast found? */
		if (_height_map_orig.height(x, y) > 15) break;

		/* Coast found in the neighborhood? */
		if (IsValidXYOrig(x + dir_y, y + dir_x) && _height_map_orig.height(x + dir_y, y + dir_x) > 0) break;
	...
	for (depth = 0; IsValidXYOrig(x, y) && depth <= max_coast_Smooth_depth; depth++, x += dir_x, y += dir_y) {
		h = _height_map_orig.height(x, y);
		h = min(h, h_prev + (4 + depth)); // coast softening formula
		_height_map_orig.height(x, y) = h;
		h_prev = h;
	}
}

/** Smooth coasts by modulating height of tiles close to map edges with cosine of distance from edge */
static void HeightMapSmoothCoastsOrig(uint8 water_borders)
{
	uint x, y;
	/* First Smooth NW and SE coasts (y close to 0 and y close to size_y) */
	for (x = 0; x < _height_map_orig.size_x; x++) {
		if (HasBit(water_borders, BORDER_NW)) HeightMapSmoothCoastInDirectionOrig(x, 0, 0, 1);
		if (HasBit(water_borders, BORDER_SE)) HeightMapSmoothCoastInDirectionOrig(x, _height_map_orig.size_y - 1, 0, -1);
	}
	/* First Smooth NE and SW coasts (x close to 0 and x close to size_x) */
	for (y = 0; y < _height_map_orig.size_y; y++) {
		if (HasBit(water_borders, BORDER_NE)) HeightMapSmoothCoastInDirectionOrig(0, y, 1, 0);
		if (HasBit(water_borders, BORDER_SW)) HeightMapSmoothCoastInDirectionOrig(_height_map_orig.size_x - 1, y, -1, 0);
	}
}
 ...
 */
static void HeightMapSmoothSlopesOrig(height_t dh_max)
{
	int x, y;
	for (y = 0; y <= (int)_height_map_orig.size_y; y++) {
		for (x = 0; x <= (int)_height_map_orig.size_x; x++) {
			height_t h_max = min(_height_map_orig.height(x > 0 ? x - 1 : x, y), _height_map_orig.height(x, y > 0 ? y - 1 : y)) + dh_max;
			if (_height_map_orig.height(x, y) > h_max) _height_map_orig.height(x, y) = h_max;
		}
	}
	for (y = _height_map_orig.size_y; y >= 0; y--) {
		for (x = _height_map_orig.size_x; x >= 0; x--) {
			height_t h_max = min(_height_map_orig.height((uint)x < _height_map_orig.size_x ? x + 1 : x, y), _height_map_orig.height(x, (uint)y < _height_map_orig.size_y ? y + 1 : y)) + dh_max;
			if (_height_map_orig.height(x, y) > h_max) _height_map_orig.height(x, y) = h_max;
		}
	}
}
 ...
 */
static void HeightMapNormalizeOrig()
{
	int sea_level_setting = _settings_game.difficulty.quantity_sea_lakes;
	const amplitude_t water_percent = sea_level_setting != (int)CUSTOM_SEA_LEVEL_NUMBER_DIFFICULTY ? _water_percent_orig[sea_level_setting] : _settings_game.game_creation.custom_sea_level * 1024 / 100;
	const height_t h_max_new = I2H(_max_height_orig[_settings_game.difficulty.terrain_type]);
	const height_t roughness = 7 + 3 * _settings_game.game_creation.tgen_smoothness;

	HeightMapAdjustWaterLevelOrig(water_percent, h_max_new);
	...
	HeightMapCoastLinesOrig(water_borders);
	HeightMapSmoothSlopesOrig(roughness);

	HeightMapSmoothCoastsOrig(water_borders);
	HeightMapSmoothSlopesOrig(roughness);

	HeightMapSineTransformOrig(12, h_max_new);

	if (_settings_game.game_creation.variety > 0) {
		HeightMapCurvesOrig(_settings_game.game_creation.variety);
	}

	HeightMapSmoothSlopesOrig(16);
}
 ...
 */
static double int_noise_orig(const long x, const long y, const int prime)
{
 ...
 */
static inline double linear_interpolate_orig(const double a, const double b, const double x)
{
	return a + x * (b - a);
}
 ...
 */
static double interpolated_noise_orig(const double x, const double y, const int prime)
{
	...
	const double v1 = int_noise_orig(integer_X,     integer_Y,     prime);
	const double v2 = int_noise_orig(integer_X + 1, integer_Y,     prime);
	const double v3 = int_noise_orig(integer_X,     integer_Y + 1, prime);
	const double v4 = int_noise_orig(integer_X + 1, integer_Y + 1, prime);

	const double i1 = linear_interpolate_orig(v1, v2, fractional_X);
	const double i2 = linear_interpolate_orig(v3, v4, fractional_X);

	return linear_interpolate_orig(i1, i2, fractional_Y);
}
...
static double perlin_coast_noise_2D_orig(const double x, const double y, const double p, const int prime)
{
	...
	for (i = 0; i < 6; i++) {
		...
		total += interpolated_noise_orig((x * frequency) / 64.0, (y * frequency) / 64.0, prime) * amplitude;
	}
...
static void TgenSetTileHeightOrig(TileIndex tile, int height)
{
 ...
 */
void GenerateTerrainPerlinOrig()
{
	uint x, y;

	if (!AllocHeightMapOrig()) return;
	GenerateWorldSetAbortCallback(FreeHeightMapOrig);

	HeightMapGenerateOrig();

	IncreaseGeneratingWorldProgress(GWP_LANDSCAPE);

	HeightMapNormalizeOrig();

	IncreaseGeneratingWorldProgress(GWP_LANDSCAPE);

	/* First make sure the tiles at the north border are void tiles if needed. */
	if (_settings_game.construction.freeform_edges) {
		for (y = 0; y < _height_map_orig.size_y - 1; y++) MakeVoid(_height_map_orig.size_x * y);
		for (x = 0; x < _height_map_orig.size_x;     x++) MakeVoid(x);
	}

	/* Transfer height map into OTTD map */
	for (y = 0; y < _height_map_orig.size_y; y++) {
		for (x = 0; x < _height_map_orig.size_x; x++) {
			int height = H2I(_height_map_orig.height(x, y));
			if (height < 0) height = 0;
			if (height > 15) height = 15;
			TgenSetTileHeightOrig(TileXY(x, y), height);
		}
	}
