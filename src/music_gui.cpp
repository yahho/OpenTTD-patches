...
{
	MusicDriver::GetActiveDriver()->SetVolume(new_vol);
}
...
static void DoPlaySong()
{
	...
	MusicDriver::GetActiveDriver()->PlaySong(filename);
	SetWindowDirty(WC_MUSIC_WINDOW, 0);
}

static void DoStopMusic()
{
	MusicDriver::GetActiveDriver()->StopSong();
	SetWindowDirty(WC_MUSIC_WINDOW, 0);
}
...
void MusicLoop()
{
	...
	if (!MusicDriver::GetActiveDriver()->IsSongPlaying()) {
...
struct MusicTrackSelectionWindow : public Window {
	MusicTrackSelectionWindow (const WindowDesc *desc, WindowNumber number) : Window(desc)
	{
		...
		switch (widget) {
			case WID_MTS_LIST_LEFT: {
				...
				for (uint i = 0; i < NUM_SONGS_AVAILABLE; i++) {
					...
					DrawString (_cur_dpi, r.left + WD_FRAMERECT_LEFT, r.right - WD_FRAMERECT_RIGHT, y, STR_PLAYLIST_TRACK_NAME);
					y += FONT_HEIGHT_SMALL;
				}
				break;
			}
...
struct MusicWindow : public Window {
	...
	MusicWindow (const WindowDesc *desc, WindowNumber number) : Window(desc)
	{
		this->InitNested(number);
	...
	virtual void DrawWidget(const Rect &r, int widget) const
	{
		switch (widget) {
			case WID_M_TRACK_NR: {
				GfxFillRect (_cur_dpi, r.left + 1, r.top + 1, r.right, r.bottom, PC_BLACK);
				...
				DrawString (_cur_dpi, r.left + WD_FRAMERECT_LEFT, r.right - WD_FRAMERECT_RIGHT, r.top + WD_FRAMERECT_TOP, str);
				break;
			}

			case WID_M_TRACK_NAME: {
				GfxFillRect (_cur_dpi, r.left, r.top + 1, r.right - 1, r.bottom, PC_BLACK);
				StringID str = STR_MUSIC_TITLE_NONE;
				if (_song_is_active != 0 && _music_wnd_cursong != 0) {
					str = STR_MUSIC_TITLE_NAME;
					SetDParamStr(0, GetSongName(_music_wnd_cursong - 1));
				}
				DrawString (_cur_dpi, r.left + WD_FRAMERECT_LEFT, r.right - WD_FRAMERECT_RIGHT, r.top + WD_FRAMERECT_TOP, str, TC_FROMSTRING, SA_HOR_CENTER);
				break;
			}
