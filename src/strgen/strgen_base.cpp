...
StringReader::StringReader(StringData &data, const char *file, bool master, bool translation) :
		data(data), file(xstrdup(file)), master(master), translation(translation)
{
}
...
static void ExtractCommandString(ParsedCommandStruct *p, const char *s, bool warnings)
{
	...
	for (;;) {
		...
		if (ar->consumes) {
			...
			p->pairs[p->np].a = ar;
			p->pairs[p->np].v = param[0] != '\0' ? xstrdup(param) : "";
			p->np++;
		}
	}
}
...
void StringReader::ParseFile()
{
	...
	MemSetT(&_lang, 0);
	bstrcpy (_lang.digit_group_separator, ",");
	bstrcpy (_lang.digit_group_separator_currency, ",");
	bstrcpy (_lang.digit_decimal_separator, ".");
