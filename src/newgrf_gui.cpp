	...
	void DrawWidget (BlitArea *dpi, const Rect &r, int widget) const OVERRIDE
	{
		...
		for (uint i = this->vscroll->GetPosition(); this->vscroll->IsVisible(i) && i < this->vscroll->GetCount(); i++) {
			...
			if (par_info->type == PTYPE_BOOL) {
				...
				std::map <uint32, GRFTextMap>::iterator iter =
						par_info->value_names.find (current_value);
				if (iter != par_info->value_names.end()) {
					const char *label = iter->second.get_string();
					if (label != NULL) {
					...
					if (this->clicked_dropdown) {
						...
						if (pt.y >= wi_rect.top && pt.y <= wi_rect.bottom) {
							...
							for (uint32 i = par_info->min_value; i <= par_info->max_value; i++) {
								*list->Append() = new DropDownListCharStringItem (par_info->value_names[i].get_string(), i, false);
							}
...
struct NewGRFWindow : public Window, NewGRFScanCallback {
	...
	virtual void OnEditboxChanged(int wid)
	{
		...
		string_filter.SetFilterTerm(this->filter_editbox.GetText());
...
struct SavePresetWindow : public Window {
		...
		presets(), vscroll (NULL), selected (-1)
	{
		...
		if (initial_text != NULL) this->presetname_editbox.Assign(initial_text);
	}
		...
		switch (widget) {
			case WID_SVP_PRESET_LIST: {
				...
				if (row < this->presets.Length()) {
					this->selected = row;
					this->presetname_editbox.Assign(this->presets[row]);
...
struct ScanProgressWindow : public Window {
	...
	void UpdateNewGRFScanStatus(uint num, const char *name)
	{
		...
		if (name == NULL) {
			...
			this->last_name = xstrdup(buf);
		} else {
			this->last_name = xstrdup(name);
		}
		this->scanned = num;
